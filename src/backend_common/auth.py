    ...
    def _require_permissions(self, permissions):
        ...
        with flask.current_app.app_context():
            if not flask_login.current_user.has_permissions(permissions):
                ...
                diff = " OR ".join(
                    [", ".join(set(p).difference(user_permissions)) for p in permissions]
                )
...
def parse_header_relengapi(request):
    ...
    if claims["typ"] == "prm":
        ...
        if token_data:
            ...
            return RelengapiTokenUser(
                claims, permissions=token_data.permissions, token_data=token_data
            )
    elif claims["typ"] == "tmp":
        now = time.time()
...
def get_taskcluster_credentials():
    ...
    return dict(
        client_id=os.environ.get(
            "TASKCLUSTER_CLIENT_ID", flask.current_app.config.get("TASKCLUSTER_CLIENT_ID")
        ),
        access_token=os.environ.get(
            "TASKCLUSTER_ACCESS_TOKEN", flask.current_app.config.get("TASKCLUSTER_ACCESS_TOKEN")
        ),
    )
...
def init_app(app):
    ...
    if app.config.get("RELENGAPI_AUTH") is True:
        app.auth_relengapi_serializer = itsdangerous.JSONWebSignatureSerializer(
    ...
    if config.get("AUTH0_AUTH") is True:
        try:
            ...
            raise backend_common.dockerflow.HeartbeatException(
                "Cannot connect to the mozilla auth0 service."
            )
